...
| `ATTACK` | 距离 ≤ 1.30 | 扣除玩家 5 点 SAN，冷却期间减速移动 |
...
| `MOSQUITO_ATTACK_SAN_DAMAGE` | 5 | 每次攻击 SAN 扣减 |
...
音效文件放入 `assets/sounds/`，缺失时 AudioManager 会 warning 一次并跳过播放。
